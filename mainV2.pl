...
if($gene_arp eq "y" || $gene_arp eq "Y"){
    ...
    for($i=0;$i<$mac_cnt;$i++){
        ...
        $mac_dst = dec2hexStr(0xFF_FF_FF_FF_FF_FF,12);
        #源MAC地址
        $mac_src = dec2hexStr(hex($base_mac)+$i,12);
        ...
        print FRAME_HEADER_FILE "$ethernet_header,$arp_protocal_head,$current_row_str,$crc\n";
...
for($i=0;$i<$row_cnt;$i++){
    ...
    #源MAC地址 (自增)
    my $mac_src = dec2hexStr($mac_src_ip,12);
    #目的MAC地址（设定为下一条数据的源MAC）
    my $mac_dst = dec2hexStr($mac_src_ip+1,12);
    ...
    print FRAME_HEADER_FILE "$ethernet_header,$ip_protocal_head,$current_row_str,$crc\n";
